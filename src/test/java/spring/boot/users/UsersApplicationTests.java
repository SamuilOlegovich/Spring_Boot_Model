...
import spring.boot.users.service.UserService;
import spring.boot.users.db.Contact;
import spring.boot.users.service.ContactService;
...
class UsersApplicationTests {
    ...
    private void inspectUsers() {
        ...
        try {
            users = this.userService.list(UserOrder.LOGIN_DESC);
        } catch (Exception exc) { users = null; }
        ...
        for (int i = 0; i < users.size(); i++) {
            ...
            switch (i) {
                case 0:
                    // visitor
                    assertTrue(u.getId() == this.userId
                                    && u.getLogin().equals("visitor")
                                    && u.isValidPassword("visitor123")
                                    && u.getEmail().equals("viktor.visitorov@lid.tv"), "Visitor contact is valid!"
                    );
                    break;
                case 1:
                    // admin
                    assertTrue(u.getId() == 1
                                    ...
                                    && u.getEmail().equals("ivan.adminov@lid.tv"), "Admin contact is valid!"
                    );
                    break;
            }
        }
    ...
    private void createContacts() {
        ...
        try {
            contacts = this.contactService.list(this.userId, ContactOrder.DEFAULT);
        } catch (Exception exc) { contacts = null; }

        assertTrue(contacts != null && contacts.size() == 0, "Contacts list is empty!");

        final Contact phoneContact = new Contact(this.userId, Contact.TYPE_PHONE,
                "+7 (000) 123-45-67", true);
        ...
        final Contact emailContact = new Contact(this.userId, Contact.TYPE_EMAIL,
                "manzherova@example.com", true);
    ...
    private void inspectContacts() {
        ...
        try {
            contacts = this.contactService.list(this.userId, ContactOrder.TYPE_DESC);
        } catch (Exception exc) { contacts = null; }
        ...
        for (int i = 0; i < contacts.size(); i++) {
            ...
            switch (i) {
                case 0:
                    // skype
                    assertTrue(c.getId() == this.skypeContactId
                                    && c.getType() == Contact.TYPE_SKYPE
                                    && c.getContact().equals("marina.manzherova"), "Skype contact is valid!"
                    );
                    break;
                case 1:
                    // email
                    assertTrue(c.getId() == this.emailContactId
                                    && c.getType() == Contact.TYPE_EMAIL
                                    && c.getContact().equals("manzherova@example.com"), "Email contact is valid!"
                    );
                    break;
                case 2:
                    // phone
                    assertTrue(c.getId() == this.phoneContactId
                                    && c.getType() == Contact.TYPE_PHONE
                                    && c.getContact().equals("+7 (000) 123-45-67"), "Phone contact is valid!"
                    );
                    break;
            }
        }
            ...
            switch (i) {
                case 0:
                    ...
                    assertTrue(c.getId() == this.emailContactId
                                    ...
                                    && c.getContact().equals("@manzherova"), "Twitter contact is valid!"
                    );
                    break;
                case 1:
                    // phone
    ...
    private void killUser() {
        ...
        try {
            users = this.userService.list(UserOrder.LOGIN_ASC);
        } catch (Exception exc) { users = null; }
